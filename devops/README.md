...
- Для рабочего окружения закомментируйте строку `caServer: https://acme-staging-v02.api.letsencrypt.org/directory`
...
- **ВАЖНО!** Закомментируйте следующие строки в traefik.yaml:
  ...
  api:
    ...
    debug: true
  ```
- Если планируете доступ к Traefik Dashboard / If you need access to the Traefik dashboard:

  - В предыдущем пункте оставьте dashboard: true, остальное false
  - > (RU) Создайте хеш пароля и обновите соответствующие настройки в dynamic.yaml \
    > (EN) Create hashed password and update settings in the dynamic.yaml
  - > (RU) Для создания хеша воспользуйтесь следующим: \
    > (EN) To create hash password use:
    ```bash
    sudo apt-get install apache2-utils
    htpasswd -Bnb username password | sed -e s/\\$/\\$\\$/g
    ```
  - Copy-paste generated hashed password to the dynamic.yaml. Add as many users as you need. And remember - all passwords should be hashed.
    ```yaml
    basicAuth:
      users:
...
- Создайте сеть Docker / Create Docker network:
  ```bash
...
- В настройках compose.portainer.yaml измените значение Host на адрес откуда будет доступен Portainer:
...
> (EN) First, use your brain 🧠, not AI, before applying any settings. AI won't be responsible for the consequences of the decisions you make. \
> (RU) Сначала включите 🧠 мозг, а не ИИ, и только потом применяйте. ИИ не будет отвечать за последствия принятых вами решений.
